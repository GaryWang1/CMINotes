...
(function ($) {
  ...
  var settings = {
    readonly: false,
    limitCount: Infinity,
    input: '<input type="text" size="55" maxLength="20" placeholder="Type and Pick Company">',
    data: [],
    searchable: true,
    searchNoData: '<li style="color:#ddd">No Company Found!</li>',
    init: noop,
    choice: noop,
    extendProps: [],
    confirm:''
  ...
  var action = {
    ...
    del: function (event) {
        ...
        if (r == true) {
            ...
            if ($target.parents(".dropdown-mul-2").length == 1)
                deleteCompany(id);
            else if ($target.parents(".dropdown-mul-3").length == 1)
                deleteMajorIndustryGroup(id);
